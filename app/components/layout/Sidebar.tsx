...
export function Sidebar({
  ...
  const discoverLink = (
    <Link
      to="/discover"
      className={cn(
        "flex items-center rounded-lg text-sm font-medium text-sidebar-accent-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring",
        discoverActive ? "bg-sidebar-accent" : "hover:bg-sidebar-accent",
        collapsed ? "size-10 justify-center" : "h-10 gap-3 px-3",
      )}
      aria-label={collapsed ? "Khảo sát học bổng" : undefined}
    >
      <span className="grid size-5 place-items-center rounded-md border border-sidebar-border text-[10px] font-bold">
        01
      </span>
      <span className={collapsed ? "sr-only" : "truncate"}>
        Khảo sát học bổng
  ...
  return (
    ...
    >
      ...
      >
        <div className={cn("pb-3", collapsed ? "px-0" : "px-2")}>
          {collapsed ? (
            <Tooltip>
              <TooltipTrigger asChild>{discoverLink}</TooltipTrigger>
              <TooltipContent side="right">Khảo sát học bổng</TooltipContent>
            </Tooltip>
          ) : (
            discoverLink
          )}
